...
<div class="d-flex justify-content-end">
	<input type="text" list="addr" id="selected" onfocus="clear()">
	<datalist id="addr">
		<option value="연트럴파크">
		<option value="홍통거리">
		<option value="홍대 걷고 싶은 거리">
		<option value="홍대 주차장 골목">
		<option value="망원시장">
		<option value="망리단길">
		<option value="용강동 음식문화거리">
	</datalist>
	<button onclick="getChart()" class="btn btn-outline-primary">확인</button>
</div>
...
function getChart(){
	selected()
	result = [];
	$.getJSON("/amchartData?addr=" + addr, function(data){
		result = data
...
function hi(){
	am5.ready(function() {
		...
		// Set data
		var data = generateDatas(36);
		console.log(result)
		series1.data.setAll(result);
		series2.data.setAll(result);
		series3.data.setAll(result);
